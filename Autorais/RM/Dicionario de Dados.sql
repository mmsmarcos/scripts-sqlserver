SELECT name FROM sys.tables 
 WHERE type_desc='USER_TABLE'
   AND name NOT IN (SELECT MASTERTABLE 
					  FROM GLINKSREL
					UNION 
					SELECT CHILDTABLE 
					  FROM GLINKSREL)
...
SELECT ForeignKeys.NAME                           [ForeignKeyName], 
  ...
  FROM sys.foreign_keys ForeignKeys JOIN sys.foreign_key_columns ForeignKeyRelationships ON ForeignKeys.object_id = ForeignKeyRelationships.constraint_object_id
 ...
 WHERE PrimaryKeyTable.name='TMOV'
